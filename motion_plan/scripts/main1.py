...
regions_ = None
state_desc_ = ['Go to point', 'Obstacle Avoid', 'Camera Control']
state_ = 0
...
def change_state(state):
    ...
    global srv_client_wall_follower_, srv_client_go_to_point_, srv_client_camera_control_
    state_ = state
...
def main():
    ...
    global srv_client_go_to_point_, srv_client_wall_follower_, srv_client_camera_control_
    
    rospy.init_node('main1')

    sub_laser = rospy.Subscriber('/laser/scan', LaserScan, clbk_laser)
    sub_odom = rospy.Subscriber('/odom', Odometry, clbk_odom)
    sub_img = rospy.Subscriber('/rahregions_ul_bot/camera/rgb/image_raw', Image, img_clbck)
    ...
    while not rospy.is_shutdown():
        ...
        if state_ == 0:
            if regions_['front'] > 0.15 and regions_['front'] < 1: # if obstacle at front
                change_state(1)
