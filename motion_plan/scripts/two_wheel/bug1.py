...
regions_ = None
state_desc_ = ['Go to point', 'follow_obstacle obstacle', 'go to closest point']
state_ = 0

follow_obstacle_starting_point_ = Point()
follow_obstacle_closest_point_ = Point()
...
def main():
    ...
    global follow_obstacle_closest_point_, follow_obstacle_starting_point_
    ...
    while not rospy.is_shutdown():
        ...
        if state_ == 0:
            if regions_['front'] > 0.15 and regions_['front'] < 1:
                follow_obstacle_closest_point_ = position_
                follow_obstacle_starting_point_ = position_
                change_state(1)
        
        elif state_ == 1:
            # if current position is closer to the goal than the previous closest_position, assign current position to closest_point
            if calc_dist_points(position_, desired_position_) < calc_dist_points(follow_obstacle_closest_point_, desired_position_):
                follow_obstacle_closest_point_ = position_
            ...
            if count_state_time_ > 5 and \
               calc_dist_points(position_, follow_obstacle_starting_point_) < 0.2:
                change_state(2)
        
        elif state_ == 2:
            # if robot reaches (is close to) closest point
            if calc_dist_points(position_, follow_obstacle_closest_point_) < 0.2:
                change_state(0)
